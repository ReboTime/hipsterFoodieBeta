...
export default function RatingIcons(props) {
	...
	return (
		<Grid item xs={12} alignItems='center' justify='space-between' container>
			{ratingIcons.map((icon) => (
				<Grid item xs={6} className={classes.ratingGrid} key={icon.name}>
					<Rating
						data-icon={icon.name}
						...
						onChange={(event) =>
							updateRating(event.currentTarget.name, +event.currentTarget.value)
						}
						precision={0.5}
						icon={icon.component}
					/>
